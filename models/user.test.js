...
const { commonBeforeAll, commonBeforeEach, commonAfterEach, commonAfterAll,
  testJobIds } = require("./_testCommon");
...
describe("applyToJob", function () {
  ...
  test("works", async function () {
    await User.applyToJob("u1", testJobIds[2], 'applied');
    const res = await db.query(`SELECT * FROM applications WHERE username='u1' AND job_id=$1`, [testJobIds[2]]);
