...
describe("POST /users", function () {
  ...
  test("unauth with not admin user", async function () {
    const resp = await request(app)
      .post("/users")
      .send({
        username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("Getting information on a user by that user", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
...
describe("DELETE /users/:username", function () {
  ...
  test("works for users not admin but that user", async function () {
    const resp = await request(app)
      .delete(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ deleted: "u1" });
...
describe("POST /users/:username/jobs/:id", () => {
  ...
  test("unauth with not admin user an different user", async function () {
    const resp = await request(app)
      .post(`/users/u2/jobs/${testJobIds[0]}`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
